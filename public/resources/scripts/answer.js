/**
 * Created by LeonDKing on 2015/12/21.
 */
...
( function ( global ) {
    ...
    require( [ 'jquery', 'common', "summernote-zh-CN", 'Ractive', 'chosen' ], function( $, common, sumNote, Ra ){
        ...
        function judgeSearch( config ){
            ...
            _.postAjax( _.commonUrlArray.getMyAnswerListUrl, params, function( data ){

                var innerDataL = _.translateDefaultValue( _.dataTransform( data.record.data ) );

                add_myAnswerOwnerStatus( innerDataL );
        ...
        //TODO 我的回答列表内增加myAnswerOwnerStatus
        function add_myAnswerOwnerStatus ( data ) {
            var filterFunc = function ( elem ) {
                for( var i in elem.answerDate ){
                    elem.answerDate[i].answerStatus == '已采纳' && elem.answerDate[i].userId == userInfo.userId ? elem[ 'myAnswerOwnerStatus' ] = true : '';
                }
                return elem;
            };
...
/*
            $( '.main_right_content_center ._date_answerC .only_pointer' ).unbind( 'click' ).bind( 'click', function () {
                ...
                $( this ).children( 'span:first-child' ).text( text == '回答' ?  '我的回答' : '回答');
                //if( text == '回答' ){
                //
