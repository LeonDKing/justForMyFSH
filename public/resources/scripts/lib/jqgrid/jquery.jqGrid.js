...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		var _pWidth = p.width,
			_pHeight = p.height,
			mwWidth = isNaN( _pWidth ) ? "auto": _pWidth + "px",
			mwHeight = isNaN( _pHeight ) ? "auto" : _pHeight + "px",	
			marginLeft = isNaN( _pWidth ) ? "auto" : ( parseInt( -_pWidth / 2, 10 ) + "px" ),
			marginTop = isNaN( _pHeight ) ? "auto" : ( parseInt( -_pHeight / 2, 10 ) + "px" );
